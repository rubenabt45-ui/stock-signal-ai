...
const generateProductResponse = (userMessage: string, intent: IntentAnalysis): DualModeAIResponse => {
  ...
  // Future Feature: Chart AI - Commented out for V1 launch
  // Chart AI feature will be re-enabled when ready
  ...
  return {
    content: `TradeIQ offers comprehensive tools for modern traders including customizable alerts for price monitoring, favorites for portfolio tracking, and AI-powered trading chat. Most features are accessible from the main dashboard.\n\nThe Pro plan unlocks advanced analytics, unlimited alerts, and priority support for serious traders.\n\nStart exploring: Try our AI trading assistant to get personalized market insights.`,
    mode: 'product',
    confidence: 0.7,
    suggestedFollowUps: [
...
const generateTradingResponse = (
  ...
  if (lowerMessage.includes('long term') || lowerMessage.includes('hold') || lowerMessage.includes('buy and hold')) {
    ...
    if (activeSymbol) {
      content += `\n\nFor ${activeSymbol}, evaluate its long-term fundamentals and technical indicators to assess entry points.`;
    }
  }
...
const generateMixedResponse = (userMessage: string, intent: IntentAnalysis): DualModeAIResponse => {
  ...
  return {
    content: `I can help you with both trading education and TradeIQ app features. Could you clarify if you're asking about investment strategies and market analysis, or how to use specific features like alerts, favorites, or navigation within the app?`,
    mode: 'mixed',
    confidence: intent.confidence,
    suggestedFollowUps: [
...
const generateDefaultResponse = (userMessage: string): DualModeAIResponse => {
  return {
    content: `Hello and welcome to the TradeIQ Assistant. I can help you with trading and investment education including strategies, technical analysis, and market insights, as well as guidance on using TradeIQ features like alerts, favorites, and portfolio management. What would you like to learn about?`,
    mode: 'mixed',
    confidence: 0.5,
    suggestedFollowUps: [
